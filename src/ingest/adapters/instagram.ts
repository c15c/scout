...
export async function instagramRecentMedia(opts: {
  ...
  const fields =
    ...
    "){followers_count,media.limit(12){caption,permalink,timestamp,media_type}}";
  const url =
    GRAPH + "/" + opts.igUserId +
...
export async function instagramOEmbed(permalink: string, appToken: string) {
  const url =
    GRAPH + "/instagram_oembed?url=" + encodeURIComponent(permalink) +
    "&omitscript=true&access_token=" + appToken;
  const res = await fetch(url);
...
export async function instagramHashtagRecent(igUserId: string, hashtag: string, token: string) {
  const searchUrl =
    GRAPH + "/ig_hashtag_search?user_id=" + igUserId +
    "&q=" + encodeURIComponent(hashtag) + "&access_token=" + token;
  const idRes = await fetch(searchUrl);
  const id = (await idRes.json()).data?.[0]?.id;
  if (!id) return [];
  const mediaUrl =
    GRAPH + "/" + id + "/recent_media?user_id=" + igUserId +
    "&fields=caption,permalink,timestamp&access_token=" + token;
