  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-line bg-panel hover:border-accent">
      ...
      {d.imageUrl ? (
        ...
        <div className="grid aspect-video place-items-center bg-canvas px-4 text-center text-[19px] font-semibold text-ink2">
          {d.title}
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        ...
        <div className="text-[14px] text-ink2">
          {d.venueName} \u00b7 {d.distanceKm} km \u00b7 {d.priceMin ? `$${d.priceMin}` : "Free"}
        </div>
        ...
        <div className="mt-auto flex items-center gap-2 border-t border-line pt-2.5">
          ...
          <div className="ml-auto flex gap-1">
            <button onClick={() => onFeedback("save")} className="h-9 w-9 rounded-[9px] border border-line">\u2606</button>
            <button onClick={() => onFeedback("down")} className="h-9 w-9 rounded-[9px] border border-line">\u2715</button>
          </div>
        </div>
      </div>
    </article>
