    ...
    <div className="flex min-h-screen">
      <aside className="hidden md:flex w-[238px] shrink-0 flex-col gap-6 border-r border-line bg-panel p-4">
        <div className="flex items-center gap-2.5 px-2 text-[19px] font-semibold tracking-tight">
          <span className="grid h-[30px] w-[30px] place-items-center rounded-[9px] bg-accent text-white">e</span>
          Scout
        </div>
      ...
      {settings && <SettingsDrawer tab={settings} onTab={setSettings} onClose={() => setSettings(null)} onRerun={() => { setSettings(null); setNeedsOnboarding(true); }} />}
