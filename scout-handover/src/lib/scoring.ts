...
export const driveMinutes = (d: number) =>
...
export type Scored = { score: number; why: string[]; distanceKm: number; driveMin: number };
...
export function score(
  ...
  const baseline = map.has("travel:close") ? 55 : 110; // a nudge, never a cap
...
export function diversify<T extends { category: string }>(rows: T[], maxRun = 2) {
...
export function learn(
  ...
  const bump = (key: string, label: string, w: number) => {
